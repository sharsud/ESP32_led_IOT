...
// --- Pattern Matrix Prototype Map (Sequenced 0 - 37) ---
void rainbow(); 
...
void setup() {
  ...
  Serial.println("==================================================================");
  Serial.println("System Core Architecture Online. 38 HIGH-READABILITY PRESETS READY.");
  Serial.println("==================================================================");
...
// ============================================================================
// ADVANCED MODULES WITH DYNAMIC COLOR PICKER INTEGRATION (28 to 37)
// ============================================================================

// 🌟 EFFECT 28: Cosmic Dust Supernova (Color Picker Tailored)
void cosmicDustSupernova() {
  ...
  if (random8() < blastChance) {
    ...
    for (int j = -radius; j <= radius; j++) {
      ...
      if (idx >= 0 && idx < dynamicNumLeds) {
        targetLeds[idx] += CRGB(255, 255, 255); // White-hot burst intensification
      }
    }
  }
}

// 🌟 EFFECT 29: Glacial Ice Melt (Color Picker Tailored)
void glacialIceMelt() {
  ...
  // Create an inverted contrast tone for the structural crack veins
  CRGB fissureColor = CRGB(255 - pickerRed, 255 - pickerGreen, 255 - pickerBlue);
  if (fissureColor == CRGB(0,0,0)) {
    fissureColor = CRGB(0, 80, 180); // Safety default deep blue if picker is pure white
  }
...
// 🌟 EFFECT 30: Matrix Digital Rain (Color Picker Tailored)
void matrixDigitalRain() {
  ...
  EVERY_N_MILLISECONDS_I(rainTimer, 30) {
    ...
    if (random8() < 140) {
      ...
      // Occasionally generate a blindingly bright head node leader particle
      if (headPos % 2 == 0) {
...
// 🌟 EFFECT 31: Interstellar Warp Drive (Color Picker Tailored)
void interstellarWarpDrive() {
...
// 🌟 EFFECT 32: Diamond Prism Shimmer (Color Picker Tailored)
void diamondPrismShimmer() {
  ...
  if (random8() < sparkleRate) {
    ...
    targetLeds[focalPixel] = CRGB::White; // Intense pure refraction flash
...
// 🌟 EFFECT 33: Magma Chamber Fissure (Color Picker Tailored)
void magmaChamberFissure() {
  ...
  for (int i = 0; i < dynamicNumLeds; i++) {
    ...
    if (pressureNoise > 195) {
      targetLeds[i] = CRGB::White; // Over-pressurized blinding hot energy discharge
    } else {
      targetLeds[i] = customMagma;
      uint8_t dynamicDimming = map(pressureNoise, 0, 195, 30, 255);
      targetLeds[i].nscale8(dynamicDimming); // Churning organic heat scaling
    }
  }
}

// 🌟 EFFECT 34: Zen Bamboo Forest (Color Picker Tailored)
void zenBambooForest() {
  ...
  for (int i = 0; i < dynamicNumLeds; i++) {
    ...
    if (swayNoise < 95) {
      targetLeds[i].nscale8(70); // Deep structural shadow background tier
    } else if (swayNoise < 175) {
      targetLeds[i].nscale8(180); // Midtone active body tier
    } else {
      targetLeds[i] += CRGB(60, 60, 60); // Dappled highlight glow tier
    }
  }
}
// --- Global Helper Function for Hardware Constraint Boundaries ---
int const_of_d(int val, int low, int high) { 
  if (val < low) {
    return low;
  }
...
// 🌟 EFFECT 35: Electric Tesla Arc (Color Picker Tailored)
void electricTeslaArc() {
...
// 🌟 EFFECT 36: Tokyo Neon Rain (Color Picker Tailored)
void tokyoNeonRain() {
  for (int i = 0; i < dynamicNumLeds; i++) { 
  ...
  // Shift the alternate droplet hue slightly away from the user's picked color to create color depth
